...
- 部署自动化现在只在隔离分支 `chore/vercel-actions` 中试运行；它只创建 Preview，不能更改正式别名。首次真实 push 验收待运行，不能把首次 CLI 成功当作自动部署通过。
...
隔离验证入口是 [`.github/workflows/vercel.yml`](../.github/workflows/vercel.yml)。它只监听 `chore/vercel-actions` 的 push；手动触发也用 job 条件拒绝其它分支。所有运行固定为 Preview，且部署命令没有 `--prod` 路径；仅本地 `git commit` 不触发。
...
同分支串行运行，不取消可能正在上传的 Preview。若排队期间已有新提交，旧构建可通过但不会替代新版本。部署 URL 和 Git SHA 写入每次 Actions 的 Job Summary。Preview 地址不能当作固定正式数据入口，也不能用于正式数据保存。
...
本分支的手动验证只可使用 Preview：`vercel pull --yes --environment=preview`、`vercel build`、`vercel deploy --prebuilt`；本机 CLI 统一追加 `--scope neutronm --global-config .trace/vercel-user`。不要在其它任务使用的共享依赖目录随意执行会重装依赖的构建；优先隔离 checkout 或 CI。
...
- Preview 分支的 Actions push 自动触发、CI 凭证权限与 prebuilt 部署仍待本次提交后实测；正式站不由这次验证发布或变更。

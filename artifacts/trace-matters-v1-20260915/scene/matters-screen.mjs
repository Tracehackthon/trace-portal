...
const ICONS = {
  ...
  work: '<rect x="3" y="6" width="18" height="13" rx="2"/><path d="M9 6V3h6v3M3 11h18M10 11v3h4v-3"/>',
  fresh: '<path d="M5 19C4 9 8 4 20 3c1 12-6 17-15 16ZM5 19l10-10"/>',
...
export function mountMattersScreen({ root, view: initialView, onAction, onHome = () => {}, assets = {}, services = {} }) {
  ...
  const brand = button('Trace', 'matters-brand', onHome);
  ...
  utilities.append(searchShortcut, el('span', 'matters-nav-divider', '|'), button('全部痕迹', 'matters-text-button', () => send({ type: 'OVERVIEW' })));
  ...
  function surface(parent, shape = BUBBLE, options = {}) {
    ...
    for (const [offset, color, opacity] of [['0%', '#ffffff', '.85'], ['34%', '#f5fffd', options.large ? '.76' : '.73'], ['77%', '#dcefe9', options.large ? '.58' : '.43'], ['100%', '#ffffff', '.78']]) fill.append(svgNode('stop', { offset, 'stop-color': color, 'stop-opacity': opacity }));
  ...
  function renderOverview() {
    ...
    for (const matter of view.matters) {
      ...
      surface(node, BUBBLE, { refract: matter.id === 'collection', width: rect[2], height: rect[3] });
      ...
      else if (matter.hasComparison || matter.id === 'collection') row.append(el('span', 'matters-change-badge matters-comparison-badge', '刚有新的对照'));
    ...
    const changed = view.matters.find(item => item.changed && item.branch);
    bird(plane, changed ? 1024 : 708, changed ? 449 : 432);
    if (changed) {
      filament(plane, ['M 1024 449 C 1070 428 1096 429 1130 421'], [[1130, 421, true]], 'matters-branch-line');
      const branch = place(button('', 'matters-branch', () => openMatter(changed)), [1146, 402, 304, 95]);
  ...
  function renderReentry() {
    ...
    center.prepend(icon('collection', 'matters-lobe-icon'));
  ...
  function relationControls(parent) {
    ...
    row.append(button('查看原文', 'matters-text-button', () => {
      const source = view.selected.sources.find(item => item.id === view.selected.comparison?.sourceId) || originalOf(view.selected);
  ...
  function renderDeep() {
    ...
    if (view.deepTab === 'comparison') {
      const eyebrow = el('p', 'matters-deep-eyebrow', '一篇与你当前理解相反的文章'); eyebrow.prepend(icon('balance'));
      ...
      body.append(eyebrow, title, columns); relationControls(body);
  ...
  function showMaterial(item, kind) {
    ...
    frame.append(close, el('p', 'matters-material-kind', `${kind === 'quote' ? '示例引文' : item.kind || '示例现场'} · 仅本次会话`), title, el('p', 'matters-material-excerpt', kind === 'quote' ? item.text : item.excerpt), el('p', 'matters-material-owner', `关联「${owner?.title || view.selected?.title || '在意的事'}」`), el('p', 'matters-material-footnote', '这是用于演示交互的现场摘录，不是已连接的外部原文。'));

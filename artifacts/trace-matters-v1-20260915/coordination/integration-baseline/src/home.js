...
let state = createState(new URLSearchParams(location.search).get('state'))
    ...
    <div class="scene" id="home-scene" data-state="${state.mode}">
      <header class="home-header">
        ...
        <nav class="home-nav" aria-label="首页导航">
          ...
          <button class="project-pill" data-action="project" type="button"><i></i><span>Codex · harness</span></button>
        </nav>
      </header>
      ...
      <form class="home-composer" id="capture-form">
        ...
        <div class="composer-bottom"><div class="source-pills"><button type="button" data-action="source">${icon('link')}知乎原文</button><button type="button" data-action="project">${icon('layers')}Codex · harness</button></div><button type="submit" class="send-orb" aria-label="留下这段想法" disabled>${icon('arrow')}</button></div>
      </form>
      ...
      <button class="profile-button" data-action="about" type="button" aria-label="关于此原型">${icon('user')}</button>
      <button class="scene-back" data-action="home" type="button" hidden>${icon('back')}收回到首页</button>
      <button class="prototype-caption" data-action="preview" type="button">交互原型 · 示例内容 · 仅本次会话</button>
    </div>
...
function duration(ms) { return reduced.matches ? 0 : ms }
...
function render(immediate=false, oldState=null) {
  ...
  for (const el of document.querySelectorAll('[data-entry]')) {
    ...
    if(!box){ el.hidden=true; continue }
    ...
    if(subtitle) subtitle.textContent=id==='work'&&state.mode==='return'?'原判断：不复制一个新的 Agent 工作界面':ENTRIES[id].subtitle
    if(id==='result') {
...
function openEntry(id) { lastFocus=$(`[data-entry="${id}"]`); dispatch({type:'OPEN',id}) }
function discussion() {
...
document.addEventListener('click',event=>{
  ...
  if(button.dataset.previewMode){dispatch({type:'MODE',mode:button.dataset.previewMode});return}
...
document.addEventListener('submit',event=>{
  ...
  if(!text)return
  if(event.target.id==='capture-form'){input.value='';event.target.querySelector('[type=submit]').disabled=true;dispatch({type:'CAPTURE',text})}
...
document.addEventListener('keydown',event=>{
  if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='k'){event.preventDefault();openUtility('search');return}
...
document.addEventListener('pointerdown',event=>{if(!$('#utility-panel').hidden&&!event.target.closest('#utility-panel,.home-nav,.profile-button,.prototype-caption'))closeUtility()})
for(const el of document.querySelectorAll('[data-entry]')) {
...
window.addEventListener('pagehide',()=>{cancelAnimations();for(const material of materials.values())material.destroy()},{once:true})

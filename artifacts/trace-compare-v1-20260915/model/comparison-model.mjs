...
function validMatter(matter) {
  return !!matter && nonempty(matter.id) && typeof matter.understanding === 'string' &&
    ...
    validFocus(matter.understanding, matter.focus) && Array.isArray(matter.links) && Array.isArray(matter.revisions);
}
function demoMatter() {
...
export function createComparisonState({ matter = demoMatter(), candidates = DEMO_CANDIDATES, sessionId = 'comparison' } = {}) {
  if (!validMatter(matter)) throw new TypeError('matter requires a current understanding, integer version, and exact nonempty focus range.');
  const catalog = [], seen = new Set();
  for (const [index, item] of (Array.isArray(candidates) ? candidates : []).entries()) {
    const candidate = normalizedCandidate(item, matter.focus, `local-${index + 1}`, candidates === DEMO_CANDIDATES);
    if (!candidate || seen.has(candidate.id)) continue;
    seen.add(candidate.id);
    // A host-specific selection must not inherit the fixture sentence as its relation target.
    if (candidate.relationship.target === DEMO_FOCUS) candidate.relationship.target = matter.focus.text;
    catalog.push(candidate);
  }
  return { schemaVersion: 1, sessionId: string(sessionId) || 'comparison', nextId: 1,
    screen: 'search', isDemo: true, notice: '', matter: clone(matter), catalog, candidateIds: [], selectedId: null,
    fixtureQuestion: matter.id === 'demo-collection',
    query: { question: matter.id === 'demo-collection' ? DEMO_QUESTION : matter.focus.text,
...
function canUndo(state) {
  ...
  if (!receipt || receipt.kind !== 'revise' || state.request) return false;
...
function requestFor(state, kind, extra = {}) {
  ...
  const payload = { kind, matterId: state.matter.id, baseVersion: state.matter.version,
    target: { field: 'understanding', ...clone(state.matter.focus) },
  ...
  switch (action.type) {
    ...
    case 'IMPORT_MATERIAL': {
      ...
      const imported = normalizedCandidate({ ...material, id: `${state.sessionId}:user-${state.nextId++}`,
        ...
        relationship: undefined, comparison: undefined }, state.matter.focus);
    ...
    case 'OPEN_REVISION':
      ...
      state.revision = { open: true, candidateId: candidate.id, draft: state.matter.focus.text,
        before: state.matter.focus.text, baseVersion: state.matter.version };
      return inform(state, '只编辑所选这一处；确认后仍需原事项提交成功。');
    case 'REVISION_DRAFT':
    ...
    case 'CONFIRM_REVISION':
      ...
      if (state.revision.baseVersion !== state.matter.version || state.revision.before !== state.matter.focus.text) return inform(state, '理解版本已变化，请重新核对这一处。');
      state.request = requestFor(state, 'revise', { before: state.revision.before, after: state.revision.draft });
      return inform(state, '等待原事项确认修改；尚未显示已更新。');
    case 'UNDO_REVISION':
      if (!canUndo(state)) return inform(state, '这次修订已不可直接撤销；不会覆盖后来的编辑。');
      state.request = requestFor(state, 'undo', { revisionId: state.receipt.revisionId });
    ...
    case 'COMMIT_RESULT': {
      ...
      if (request.kind === 'revise') {
        ...
        return inform(state, '原事项已确认：只更新这一处，材料作为本次修订依据保留。');
      }
      state.screen = 'compare';
...
function validReceipt(receipt, request, matter) {
  if (!receipt || receipt.requestId !== request.id || receipt.kind !== request.kind || receipt.matterId !== request.matterId ||
    receipt.requestFingerprint !== fingerprint(request) || receipt.beforeVersion !== request.baseVersion ||
...
export function selectComparisonView(state) {
  ...
  return { screen: state.screen, isDemo: true, notice: state.notice,
    matter: { id: state.matter.id, title: state.matter.title, understanding: state.matter.understanding,
      version: state.matter.version, focus: clone(state.matter.focus), unresolved: clone(state.matter.unresolved ?? '') },
    ...
    revision: { open: state.revision.open, draft: state.revision.draft, before: state.revision.before, after: state.revision.draft,
      baseVersion: state.revision.baseVersion, canConfirm: state.revision.open && !state.request && nonempty(state.revision.draft) &&
        ...
        state.revision.before === state.matter.focus.text && nonempty(candidate?.relationship.target) },
...
function linkFor(matter, request) {
  ...
  const existing = matter.links.find((link) => link.sourceId === request.source.id &&
    link.focus?.start === request.target.start && link.focus?.end === request.target.end && link.focus?.text === request.target.text &&
  ...
  const link = { id: `comparison-link:${request.id}`, sourceId: request.source.id, source: clone(request.source),
    focus: clone(request.target), baseVersion: request.baseVersion, relationship: clone(request.relationship), note: string(request.note), requestId: request.id };
...
export function applyComparisonRequest(currentMatter, request) {
  ...
  if (!request || !nonempty(request.id) || !['link', 'revise', 'undo'].includes(request.kind)) return fail(currentMatter, 'invalid_request', '无法识别这次提交，未进行修改。');
  ...
  if (request.baseVersion !== currentMatter.version) return fail(currentMatter, 'version_conflict', '原理解已有后来修改；这次未覆盖它，请重新核对。');
  if (request.target?.field !== 'understanding' || !validFocus(currentMatter.understanding, request.target) ||
    request.target.start !== currentMatter.focus.start || request.target.end !== currentMatter.focus.end || request.target.text !== currentMatter.focus.text)
    return fail(currentMatter, 'focus_conflict', '原选中片段已变化，未按旧位置覆盖。');
  ...
  if (request.kind === 'undo') {
    ...
    if (request.kind === 'revise' && (request.before !== request.target.text || !nonempty(request.after) || request.after === request.before))
    ...
    if (request.kind === 'revise') {
      ...
      matter.revisions.push({ id: receipt.revisionId, kind: 'comparison', requestId: request.id,
        target: clone(request.target), before: request.before, after: request.after,

...
export function mountComparisonScreen({root,view,onAction=()=>{},onReturn=()=>{},onContinue=()=>{},onAll=()=>{},assets={},services={}}) {
  ...
  for (const [key,name] of [['serifFont','Trace Compare Serif'],['sansFont','Trace Compare Sans']]) {
    ...
    const face = new FontFace(name,`url(${JSON.stringify(String(assets[key]))})`,{weight:'100 900'});
  ...
  function revisionModal() {
    modal('确认局部修订',`<p class="compare-dialog-help">先看差异，再确认写回。接入材料与修改理解是两件不同的事。</p><section class="compare-dialog-before"><h3>原来的这一处</h3><p data-modal="before"></p></section><label for="compare-revision">修改为</label><textarea id="compare-revision" data-field="revision" rows="5"></textarea><section class="compare-dialog-after"><h3>确认后的这一处</h3><p data-modal="after"></p></section><p class="compare-dialog-help">此次只替换选中片段，不重写其他内容。</p><p class="compare-dialog-version"></p><div class="compare-dialog-actions">${button('cancel-revision','取消修改')}${button('confirm-revision','确认更新这一处','compare-primary')}</div>`, 'revision');
    $('[data-modal="before"]').textContent=asText(current.revision?.before);
  ...
  function update(nextView) {
    ...
    all('[data-text="focus"]').forEach(el=>emphasizeTerm(el,'必须'));
    ...
    if(screen==='search') {
      ...
      $('[data-search-submit]').disabled=!current.query?.question?.trim() || !!current.pending;
    }
    ...
    if(screen==='returned'){
      ...
      text('receiptStatus',receipt?'我的理解 · 已更新':'尚无已提交修订');
      text('before',before);text('after',after);text('sourceTitle',source?.title || receipt?.sourceTitle || item?.title || '这次对照');
      if(!asText(after).includes('必须'))emphasizeTerm($('.compare-before p'),'必须','s');
  ...
  function submit(event) {
    ...
    if(form.dataset.form==='search')emit({type:'SEARCH'});

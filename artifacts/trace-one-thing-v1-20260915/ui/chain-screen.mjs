...
export function mountChainScreen({root, view, onAction, onHome, onMatters, assets={}, services={}}) {
  ...
  let lastScreen=null, lastId=null, glass=null, animation=null, returnFocus=null;
  ...
  const fonts=[];
  for (const [family,url] of [['Trace Chain Serif',assets.serifFont],['Trace Chain Sans',assets.sansFont]]) {
    if (url && typeof FontFace === 'function') {
      const font = new FontFace(family,background(url)); fonts.push(font);
      font.load().then(f => { if (!disposed) document.fonts.add(f); }).catch(() => {});
    }
  }
  ...
  const title = () => current.matter?.title || '还没有命名的这一点';

  function header() {
    const simple = current.screen==='paused' || current.screen==='handoff';
    return `<header class="chain-header" data-key="header"><button type="button" data-action="home" class="chain-brand" aria-label="Trace 首页"><span>${mark}</span>Trace</button>${!simple ? `<div class="chain-breadcrumb"><button data-action="matters">在意的事</button><span>/</span><span>${h(title())}</span></div>` : ''}<nav>${simple ? btn('matters','全部事项','layers','bare') : btn('provenance','看看来路','search','bare')}${btn('collapse','收起','collapse','bare',current.selectedId ? '' : 'disabled')}</nav></header>`;
  }
  function scenery() {
  ...
  function common(content,{wide=false,compose=true}={}) {
    return `<main class="chain-main ${wide?'chain-main-wide':''}" data-key="main"><div class="chain-heading" data-key="heading"><h1>${h(title())}<b>。</b></h1>${TITLES[current.screen] ? `<p>${TITLES[current.screen]}</p>`:''}</div><section class="chain-panel chain-glass" data-key="panel">${content}</section>${compose ? composer(current.screen==='revised'?'继续写，或带来下一次经历……':undefined) : ''}</main>`;
  }
  function resume() {
    const context=current.context||{};
    return common(`<div class="chain-panel-scroll">${current.contextMode==='fresh' ? `<div class="chain-fresh">本次未带入旧理解；来源和原话仍可按需查看。${btn('resume-context','恢复原来的上下文','undo','bare')}</div>` : row('当时为什么在意',context.whyCare || current.matter?.originalText || '从留下的原话开始。')}${row('上次停在这里',context.stop || '尚未留下停点。','stop',firstSource() ? btn('source','原文片段 · 查看原现场','file','subtle') : '')}</div><div class="chain-panel-actions">${btn('discussion','拿两个情形比一比','balance')}${btn('understanding','我想先自己写','pen')}${btn(current.contextMode==='fresh'?'resume-context':'fresh',current.contextMode==='fresh'?'恢复原来的上下文':'这次先不带入旧理解',null,'text-link')}</div>`);
  }
  function discussion() {
  ...
  function revised() {
    const m=current.matter||{};
    return common(`<div class="chain-panel-scroll"><div class="chain-editor-heading"><span class="chain-round">${icon('file')}</span><h2>我的理解</h2><span class="chain-muted">个人草稿 · 已保存于本次会话</span></div><p class="chain-revised-text">${h(m.understanding || '暂无已保存的理解。')}</p>${row('还没分清',m.stop || '尚未留下停点。','question')}<div class="chain-two-actions">${btn('revision-history','看看这次改了哪里','file')}${btn('result-history','查看使用经历','undo')}</div><div class="chain-inference">${icon('bulb')}已补上这次经历的条件，未写成通用结论。</div></div><div class="chain-panel-actions">${btn('understanding','继续写','pen','subtle')}${btn('handoff','带去用','link','subtle')}${current.undo?.revision ? btn('undo-revision','撤销修订','undo','text-link'):''}</div>`,{wide:true});
  }
  function reading() {
    return `<main class="chain-reading" data-key="main"><header class="chain-reading-header"><div class="chain-window-dots"><i></i><i></i><i></i></div><span>${icon('back')}</span><span class="chain-address">${icon('file')}示例阅读页</span>${btn('home','返回 Trace',null,'bare')}</header><article class="chain-article"><small>示例文章</small><h1>为什么收藏之后，很少再回来？</h1><p class="chain-article-meta">约四分钟阅读 · 这是用于原型的示例文章，不是当前浏览器内容。</p><div data-selection="article" data-key="article-text" class="chain-article-text">${ARTICLE.map(p=>`<p>${h(p)}</p>`).join('')}</div><small>选中文章中的一段，再留下一点自己的想法。</small></article><button class="chain-floating-entry" data-action="toggle-reading" aria-label="打开 Trace 留下一点">${mark}</button>${readingOpen?`<section class="chain-capture chain-glass" data-key="capture"><div class="chain-side-heading"><span class="chain-small-brand"><span>${mark}</span>Trace</span>${btn('toggle-reading','关闭','close','icon-only')}</div><div class="chain-capture-source">${current.capture?.excerpt ? `<blockquote>${h(current.capture.excerpt)}</blockquote>` : ''}${(current.availableSources||[]).slice(0,1).map(s=>btn('toggle-source',current.capture?.sourceIds?.includes(s.id)?'原文片段 · 已带入':'带入原文来源','file','subtle',`data-source="${h(s.id)}" aria-pressed="${current.capture?.sourceIds?.includes(s.id)||false}"`)).join('')}</div>${field('capture',current.capture?.text,'选中片段与此刻想法','chain-capture-input','rows="4"')}<p class="chain-footnote">不用起标题，也可以先不整理。</p><footer>${btn('capture-discuss','展开聊聊',null,'',canCapture()?'':'disabled')}${btn('capture-leave','留一下','bookmark','primary',canCapture()?'':'disabled')}</footer></section>`:''}</main>`;
  }
  ...
  function modalHTML() {
    ...
    if (modal.type==='provenance') { heading='原文与来路'; body=`<h3>原话</h3><blockquote>${h(m.originalText||'尚未留下原话。')}</blockquote><h3>来源材料</h3>${(m.sources||[]).map(s=>`<details><summary>${h(s.title)}</summary><p>${h(s.excerpt)}</p></details>`).join('')||'<p>暂无来源。可以粘贴一段材料。</p>'}<h3>对照关系</h3>${(m.observations||[]).map(o=>`<p>${h(RELATIONS[o.relation]||o.relation)} · ${h(o.text)}</p>`).join('')||'<p>暂无已确认对照。</p>'}<h3>留下的旁支</h3>${(m.branches||[]).map(b=>`<blockquote>${h(b.text)}<small>来处：${h(b.origin?.text || b.origin?.field || '')}</small></blockquote>`).join('')||'<p>暂无旁支。</p>'}`; }
    ...
    if (modal.type==='result-history') { heading='查看使用经历'; body=(m.results||[]).map(r=>`<section class="chain-history-item"><h3>事实</h3><p>${h(r.fact)}</p><h3>解释</h3><p>${h(r.interpretation)}</p><h3>未确认</h3><p>${h(r.unconfirmed)}</p></section>`).join('')||'<p>没有自动生成使用反馈。</p>'; }
    if (modal.type==='suggest') { heading='建议修改 · 只改这一处';body=`<label>原句<blockquote>${h(current.focus?.text || '')}</blockquote></label><label>建议替换文字${field('suggestion-draft',suggestionDraft,'替换为……','','rows="4"')}</label><p class="chain-footnote">示例建议由你确认后才改写选中的文字。</p>${btn('create-suggestion','生成示例建议','pen','primary',suggestionDraft.trim()?'':'disabled')}`; }
    if (modal.type==='material') { heading='粘贴材料摘录';body=`<p>原型只支持文字摘录，不会上传附件。</p>${field('material-draft',materialDraft,'把原文粘贴在这里……','','rows="6"')}${btn('add-material','将摘录带入新材料','link','primary',materialDraft.trim()?'':'disabled')}`; }
  ...
  function render() {
    ...
    fresh.innerHTML=`${!['reading','work'].includes(current.screen)?header():''}${scenery()}${renderer()}<footer class="chain-prototype-label" data-key="truth">交互原型 · 示例内容 · 仅本次会话</footer>${current.notice && current.screen!=='paused' ? `<div class="chain-notice" data-key="notice" role="status"><span>${h(current.notice)}</span>${btn('clear-notice','关闭','close','icon-only')}</div>`:''}${modalHTML()}`;
  ...
  function selectText(element) {
    ...
    else dispatch({type:'FOCUS',field:name,...selection});
  }
  listen(stage,'click',event=>{
    ...
    if(action==='comparison'){dispatch({type:'OPEN_COMPARISON',sourceId:(current.availableSources||[]).find(s=>s.kind==='example-comparison')?.id || firstSource()?.id});return;}
  ...
  listen(stage,'keydown',event=>{
    if(event.key==='Escape'){event.preventDefault();if(modal)closeModal();else if(current.focus)dispatch({type:'CLEAR_FOCUS'});else if(['comparison','handoff'].includes(current.screen))dispatch({type:'BACK'});return;}
  ...
  return {
    ...
    destroy(){if(disposed)return;disposed=true;abort.abort();observer.disconnect();glass?.destroy();animation?.cancel?.();for(const font of fonts)document.fonts.delete(font);root.replaceChildren();root.classList.remove('chain-root');root.style.removeProperty('--chain-background');},
  };
}

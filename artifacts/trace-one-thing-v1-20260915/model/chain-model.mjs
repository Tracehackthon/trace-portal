...
function blankHandoff() {
  return { destination: { agent: 'Codex', project: 'harness', task: '收藏入口原型' }, selectedText: '', selectionEdited: false, role: 'trial', note: '', scope: 'current-task', confirmed: false, understandingVersion: 0, evidenceLevel: 'none' };
}
...
function validFocus(m, s, focus) {
  if (!focus || !['discussion', 'understanding'].includes(focus.field)) return false;
  const text = focus.field === 'understanding' ? m.understandingDraft : discussionText(s);
...
export function reduceChain(previous, action = {}) {
  ...
  switch (action.type) {
    ...
    case 'FOCUS': {
      const focus = { field: action.field, start: action.start, end: action.end, text: string(action.text) };

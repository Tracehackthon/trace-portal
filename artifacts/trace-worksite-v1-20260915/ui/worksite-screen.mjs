...
export function mountWorksiteScreen({root,view,onAction,onHome,onOpenMatter,onOpenArtifact,onReturnToAgent,assets={},services={}}={}) {
    ...
    <div class="worksite-viewport">
      <div class="worksite-scene">
        <header class="worksite-header">
          ...
          <nav aria-label="当前工作操作">${button('switch','切换工作',{glyph:'switch'})}<span class="worksite-nav-divider"></span>${button('agent','回到 Codex',{glyph:'box',cls:'worksite-agent-button'})}${icon('external')}</nav>
        </header>
        ...
        <div class="worksite-state" data-screen="overview">
          ${surface('intake-list','worksite-overview-intake',`<h2>本次带入 <span data-text="intake-count"></span></h2><div class="worksite-intake-list"></div><div class="worksite-empty-intake" hidden><h3>还没有本次带入</h3><p>先从一件事中选择要带去用的理解；这里不会自动选中全部内容。</p></div>`)}
  ...
  for(const [kind,path] of [['serif',assets.serifFont],['sans',assets.sansFont]]) {
    if(path && typeof FontFace==='function') {
      ...
      const face=new FontFace(family,`url(${JSON.stringify(String(path))})`,{display:'swap'});
  ...
  function openArtifact(artifact=current.decision?.artifact){
    if(!artifact?.url || typeof onOpenArtifact!=='function'){showNotice('示例原型尚未连接真实产物。');return;}
    callExternal(onOpenArtifact,artifact,'示例原型尚未连接真实产物。');
  }
  ...
  function openPanel(name){
    ...
    let title='',html='';
    if(name==='switch'){
      title='切换工作';html='<p>选择一个工作后，带入、发现和结果会分别保留。</p><div class="worksite-work-list"></div><p class="worksite-panel-note">外部 Agent 尚未连接。当前只是独立交互模块，不表示已送达、已实现或已验证。</p>';
    } else if(name==='source') {
      ...
      title='个人区域';html='<p>当前没有账户接入。</p><p>只保存本次会话，刷新后重置。</p>';
  ...
  function update(nextView){
    ...
    q('.worksite-agent-button span').textContent=`回到 ${current.work?.agent||'Agent'}`;
    text('intake-count',`· ${current.intake?.length||0}`);
    const decision=current.decision||{};
    text('decision-title',decision.title||'暂无具体取舍');text('decision-short',decision.title||'暂无具体取舍');text('decision-description',decision.description||'外部 Agent 尚未连接，不会自动生成工作影响。');
    ...
    text('observation',current.impact?.unconfirmed?.[0]||'仍需观察：几天后能否接回当时的想法');
    qa('[data-action="artifact"] span').forEach(el=>{if(el.closest('.worksite-artifact-link'))el.textContent=decision.artifact?.title||'保存交互原型';});
    ...
    text('artifact-demo-title',`${decision.artifact?.title||'保存交互原型'}${decision.artifact?.isDemo&&!decision.artifact.title.includes('示例')?' · 示例':!decision.artifact?.isDemo&&!decision.artifact?.url?' · 尚未连接':''}`);
    ...
    field('result-matter',result.matterId,true);text('matter-stop',matter?.stop||'暂未选择一件事');text('matter-understanding',matter?`原先的理解：${matter.understanding}`:'选择一件事，再对照它的理解。');
    ...
    const hasFact=Boolean(result.fact?.trim());setDisabled('[data-action="keep-result"],[data-action="retry"]',!hasFact);setDisabled('[data-action="review"]',!hasFact||!result.proposedUnderstanding?.trim()||!matter);
    ...
    text('receipt',receipt?(receipt.undone?'已撤销，结果仍保留':`已确认修改 · 版本 ${receipt.version}`):current.retry?'再试计划已留在本次会话，不会自动创建外部任务。':'已保存结果，理解未变');
  ...
  function onClick(event){
    ...
    else if(type==='agent'){if(!current.work?.connected||typeof onReturnToAgent!=='function')showNotice('此工作尚未连接外部 Agent。');else callExternal(onReturnToAgent,current.work,'此工作尚未连接外部 Agent。');}

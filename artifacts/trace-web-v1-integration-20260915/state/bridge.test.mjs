...
import { createBridge, captureInput, dispatchChain, selectChain, openComparison, dispatchComparison,
  ...
  createWorkFromHandoff, dispatchWorksite, selectWorksite } from './bridge.mjs';
...
test('original/discussion basis is explicitly unconnected, never promoted into understanding', () => {
  const h = captured();
  for (const field of ['originalText', 'discussion']) {
    const attempted = openComparison(h, { sessionId: 'original', matterId: ID, anchor: { field, text: ORIGINAL, start: 0, end: ORIGINAL.length, baseVersion: 1 } });
    assert.equal(attempted.error.code, 'unsupported_basis');
    assert.equal(current(attempted).understanding, '');
    assert.equal(current(attempted).understandingVersion, 0);
